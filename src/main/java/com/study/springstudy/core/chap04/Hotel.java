...
/**
 * @solution -  객체 생성의 제어권을 Spring에게 모두 넘긴 후
 *              어노테이션 문법을 이용하여 객체 생성 및 빈 등록 로직을 대폭 감소
 *
 * @Component - 스프링 컨테이너에 빈(bean)으로 등록할 객체이다,.
 * @Autowired - 필드, 생성자, setter에 각각 적용 가능.
 *              특정 객체가 다른 객체와의 의존성을 가질 때,
 *              의존성 주입을 자동으로 처리해 주는 문법
 *              생성자에 붙이는 것을 권장.
 * @Qualified - 틑정 타입을 만족하는 빈이 2개 이상일 경우,
 *              스프링이 자제적으로 무엇을 주입할 지 판단할 수 없기 때문에,
 *              별칭ㄹ 등을 이용해서 주입할 빈을 직접 지목하는 문법.
 *
 *              제어의 역전(IoC: Inversion of Control): 객체 생성의 제어권를 외부에서 처리하는 것.
 *              의존성 주입(Di: Dependency Injection): 외부에서 생성된 객체를 주입받는 개념.
 *              
 * @problem -   추상화를 했지만 여전히 의존객체를 바꾸려면 코드를
 *              직접 변경해야 한다.
 */

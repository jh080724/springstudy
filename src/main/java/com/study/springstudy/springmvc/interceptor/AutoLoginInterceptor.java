...
public class AutoLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (autoLoginCookie != null) {
            ...
            //4.DB에서 쿠키가 가지고 있는 SessionId와 동일한 값을 가진 회원 조회
            Member member = memberMapper.findByCookie(sessionId);

            // 5. 회원이 정상적으로 조회가 되었다면, and 자동 로그인 만료시간 이전이라면 로그인을 수행
            if (member != null &&
        ...
        // 자동 로그인 신청했던 사람이든, 아니든 간에
        // 요청은 무조건 컨트롤러로 전달되어야 하니까, 리턴 값은 true로 고정
        return true;
    }
}

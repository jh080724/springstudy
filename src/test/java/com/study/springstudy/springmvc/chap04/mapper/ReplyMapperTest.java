...
class ReplyMapperTest {
    ...
    @Test
    @DisplayName("77번 게시물의 댓글 목록을 조회햇을때 결과 리스트의 size는 n이어야 한다.")
    void findAllTest() {
        // given
        int boardNo = 77;
        ...
        // then
        Assertions.assertEquals(replyMapper.count(boardNo), replyList.size());
    }

    @Test
    @DisplayName("77번 게시물의 댓글 중 n번 댓글을 삭제하면 " +
            "n번 댓글은 조회되지 않아야 하고, 77번 을 전체 조회하면" +
            " 리스트 사이즈가 하나 줄어 있어야 한다.")
    @Transactional
    void deleteTest() {
        // given
        int boardNo = 77;
        ...
        Assertions.assertNotEquals(beforeDeleteCount, replyMapper.findAll(boardNo).size());
    }

    @Test
    @DisplayName("103번 댓글의 댓글 내용을 수정하면 다시 조회했을때 수정된 내용이 조회되어야 한다.")
    void modifyTest() {
        // given
        int replyNo = 103;
        String newReplyText = "수정댓글댓글~~~";
        Reply reply = Reply.builder()
                .replyNo(replyNo)
